      ...
      lessons: [
        {
          ...
          slides: [
            ...
            ask('trigid-zn-plot'),
            teach(
              { kind: 'prose', text: 'De Moivre says $z^n$ is the same point at angle $n\\theta$, so the same pairing works for any power:' },
              { kind: 'display', tex: 'z^n + z^{-n} = 2\\cos n\\theta' },
              { kind: 'display', tex: 'z^n - z^{-n} = 2i\\sin n\\theta' },
              { kind: 'prose', text: 'With $\\theta = \\tfrac{\\pi}{9}$, $z^3 + z^{-3} = 2\\cos\\tfrac{\\pi}{3} = 1$.' },
            ),
            ...
            teach(
              ...
              { kind: 'prose', text: 'That is $4\\cos 3\\theta\\cos\\theta = 2\\cos 4\\theta + 2\\cos 2\\theta$, so' },
              { kind: 'display', tex: '\\begin{aligned} & 2\\cos 3\\theta\\cos\\theta \\\\ &= \\cos 4\\theta + \\cos 2\\theta \\end{aligned}' },
            ),
            ask('trigid-product-sum'),
...

        {
          ...
          slides: [
            ...
            teach(
              { kind: 'prose', text: 'A product like $\\cos^2\\theta\\sin^2\\theta$ multiplies both brackets together. Before you expand, you can already say what comes out:' },
              { kind: 'prose', text: 'An even power of $\\sin\\theta$ makes the $i$s real, so the answer is cosines. An odd power leaves one $i$, so it is sines. The largest multiple is the total power.' },
              { kind: 'display', tex: '8\\cos^2\\theta\\sin^2\\theta = 1 - \\cos 4\\theta' },
            ),
            ask('trigid-power-form', 2),
            ask('trigid-power-flow'),
...

        {
          ...
          slides: [
            ...
            teach(
              { kind: 'prose', text: 'A power of $\\cos x$ has no simple antiderivative, but each multiple angle does. Rewrite, then integrate term by term:' },
              { kind: 'display', tex: '\\begin{aligned} \\cos^4 x &= \\tfrac{1}{8}\\cos 4x \\\\ &\\quad + \\tfrac{1}{2}\\cos 2x + \\tfrac{3}{8} \\end{aligned}' },
              { kind: 'prose', text: 'So $\\int \\cos^4 x\\,dx = \\tfrac{1}{32}\\sin 4x + \\tfrac{1}{4}\\sin 2x + \\tfrac{3}{8}x + C$.' },
            ),
...

        {
          ...
          slides: [
            ...
            teach(
              { kind: 'prose', text: 'Doing that to both brackets, the $2i$s cancel and what is left is real apart from one power of $e$. Its real part is the closed form:' },
              { kind: 'display', tex: '\\sum_{k=1}^{n} \\cos k\\theta = \\frac{\\sin\\frac{n\\theta}{2}\\cos\\frac{(n+1)\\theta}{2}}{\\sin\\frac{\\theta}{2}}' },
              { kind: 'prose', text: 'The imaginary part gives the sum of sines, with $\\sin\\frac{(n+1)\\theta}{2}$ in place of the cosine.' },
            ),
            ask('trigid-series-value'),

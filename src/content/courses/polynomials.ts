      ...
      lessons: [
        {
          ...
          slides: [
            teach(
              ...
              prose(
                'The remainder on dividing by $(x - a)$ is $p(a)$, found with no division at all. It is also the last number synthetic division ends on. Dividing $x^{3} + 2x^{2} - 5x + 3$ by $(x - 2)$:',
              ),
              working('p(2) &= 8 + 8 - 10 + 3', '&= 9'),
              prose('So the remainder is $9$.'),
            ),
            ask('poly-remainder'),
            ask('poly-synthetic-tree'),
      ...
      lessons: [
        {
          ...
          slides: [
            ...
            asking(
              'poly-root-signs',
              1,
              prose('The signs of the roots can be read without solving. A positive product means the same sign, and the sum says which. For $x^{2} + 7x + 10$:'),
              working('\\alpha\\beta &= 10 > 0', '\\alpha + \\beta &= -7 < 0'),
              prose('Same sign, adding to a negative number: both negative. They are $-2$ and $-5$. A negative product, as in $x^{2} + x - 6$, means one positive and one negative root.'),
            ),
        ...
        },
        {
          ...
          slides: [
            ...
            asking(
              'poly-which-identity-flow',
              1,
              prose('If only the $x$ coefficient is known, use the sum of the pairs. $x^{3} + kx^{2} - 5x + m = 0$ has roots $1$ and $3$, so $\\Sigma\\alpha\\beta = -5$. The pairs are $1 \\times 3$, then $\\gamma$ with each of $1$ and $3$:'),
              working('1 \\times 3 + \\gamma(1 + 3) &= -5', '3 + 4\\gamma &= -5', '\\gamma &= -2'),
            ),
      ...
      lessons: [
        {
          ...
          slides: [
            ...
            teach(
              prose('The same answer comes from comparing coefficients. Divide $x^{4} + 3x^{3} + 3x^{2} + 2x - 5$ by $x^{2} + 2x + 3$, writing it as'),
              working('&(x^{2} + 2x + 3)(ax^{2} + bx + c)', '&\\quad + rx + s'),
              prose('Match the powers from the top down. Each collects from the pairs that make it: $x^{2}$ gets $c$, $2x \\times bx$ and $3 \\times ax^{2}$.'),
              working(
                'x^{4}\\!: \\;\\; a &= 1',
                'x^{3}\\!: \\;\\; b + 2 &= 3, \\;\\; b = 1',
                'x^{2}\\!: \\;\\; c + 2 + 3 &= 3, \\;\\; c = -2',
                'x\\!: \\;\\; r - 4 + 3 &= 2, \\;\\; r = 3',
                '\\text{number}\\!: \\;\\; s - 6 &= -5, \\;\\; s = 1',
              ),
              prose('So the quotient is $x^{2} + x - 2$ and the remainder is $3x + 1$.'),
            ),
        ...
        },
        {
          ...
          slides: [
            ...
            teach(
              prose('A negative leading coefficient carries through each division. $p(x) = -x^{4} - x^{3} + 7x^{2} + x - 6$ has $p(1) = -1 - 1 + 7 + 1 - 6 = 0$, and the cubic left is zero at $x = 2$:'),
              working(
                '&(x - 1)(-x^{3} - 2x^{2} + 5x + 6)',
                '=\\;&(x - 1)(x - 2)',
                '&\\quad \\times (-x^{2} - 4x - 3)',
              ),
              prose('Take the minus out of the last bracket, then factorise what is left:'),
              maths('-(x - 1)(x - 2)(x^{2} + 4x + 3) = -(x - 1)(x - 2)(x + 1)(x + 3)'),
              prose('The minus stays in front, and the solutions are $x = -3,\\ -1,\\ 1,\\ 2$. A root found twice is a repeated factor, written with a square, and it is one solution, not two.'),
            ),
      ...
      lessons: [
        {
          ...
          slides: [
            ...
            teach(
              ...
              prose('A squared bracket is never negative, so the sign does not change at its root. $(x - 1)^{2}(x + 2)$ is $+$ on the far right, stays $+$ past $1$, and changes past $-2$:'),
              maths('{-}\\quad{+}\\quad{+}'),
            ),
        ...
        },
        {
          ...
          slides: [
            teach(
              ...
              prose('On a number line, shade those two stretches. At the critical values $p(x) = 0$, which is not greater than $0$, so the dots there are hollow. $\\le 0$ wants the negative stretches, and lets $p(x) = 0$ in, so its dots are filled:'),
            ...
            ask('poly-ineq-flow'),
            teach(
              prose('Strict or not also decides the whole numbers at the ends. In $x \\le -2$ or $1 \\le x \\le 3$, the largest whole number is $3$. For $< 0$ the set is $x < -2$ or $1 < x < 3$, which leaves $3$ out, so its largest is $2$.'),
              prose('Both sets run off to the left, so neither has a smallest whole number. The $> 0$ set, $-2 < x < 1$ or $x > 3$, runs off to the right instead: no largest, and its smallest whole number is $-1$.'),
            ),
            ...
            teach(
              ...
              prose('This set runs off to the left, so it has a largest whole number, $4$, and no smallest.'),
            ),
        ...
        },
        {
          ...
          slides: [
            ...
            teach(
              prose('A touching root leaves a hole or a lone point in a set. $(x - 1)^{2}(x + 2)$ is negative left of $-2$, positive either side of $1$, and $0$ at $1$. So:'),
              working(
                '> 0\\!: &\\;\\; x > -2 \\text{ and } x \\ne 1',
                '\\ge 0\\!: &\\;\\; x \\ge -2',
                '\\le 0\\!: &\\;\\; x \\le -2 \\text{ or } x = 1',
                '< 0\\!: &\\;\\; x < -2',
              ),
              prose('$> 0$ shades from $-2$ on, with a hollow dot at $1$: a hole. $\\le 0$ leaves out both sides of $1$ but lets $1$ itself in: a lone filled dot.'),
            ),
            ask('poly-hole-flow'),
            ask('poly-touch-tiles'),
            ask('poly-touch-line'),
            teach(
              prose('A minus in front turns every sign round, squared bracket or not. $-(x - 1)^{2}(x + 2)$ has the signs $+\\;-\\;-$, so'),
              maths('-(x - 1)^{2}(x + 2) \\ge 0'),
              prose('is $x \\le -2$ or $x = 1$. Both sides of $1$ are negative, but at $1$ it is $0$: a lone filled dot again.'),
            ),
        ...
        },
        {
          ...
          slides: [
            ...
            teach(
              prose('Now it is a factorised cubic against $0$, $(x + 3)(x + 1)(x - 2) > 0$, and the set is $-3 < x < -1$ or $x > 2$.'),
              prose('If the cubic leads with $-x^{3}$, its brackets have a minus in front. Multiply both sides by $-1$ to clear it, which turns the inequality round:'),
              working('-(x + 1)(x - 2)(x - 4) &\\ge 0', '(x + 1)(x - 2)(x - 4) &\\le 0'),
              prose('A positive number in front, such as the $2$ in $-2(x + 1)(x - 2)(x - 4)$, divides out and changes nothing.'),
            ),
            ask('poly-flip-choice'),
            ask('poly-one-side-steps', 2),
            ask('poly-rearrange-line', 2),
            teach(
              prose('A bracket written $3 - x$ is $-(x - 3)$, so it brings a minus too. Count them all: an odd number turns the inequality round, an even number does not.'),
              working('2(3 - x)(x + 1)(2 - x) &\\le 0', '2(x - 3)(x + 1)(x - 2) &\\le 0', '(x - 3)(x + 1)(x - 2) &\\le 0'),
              prose('Two minuses, so nothing turned round, and the $2$ divided out. Never divide both sides by a bracket, though: its sign changes with $x$, so you cannot tell whether to turn round.'),
            ),
      ...
      lessons: [
        ...
        },
        {
          ...
          slides: [
            ...
            teach(
              ...
              prose('So when one cut is given, the other is the root left once you set aside the given one and the one outside the range: given $2$ here, it is $4$.'),
            ),
        ...
        },
        {
          ...
          slides: [
            ...
            teach(
              prose('To pick a cubic from a table, check each candidate against it. Here $y = 0$ at $x = 1$ and $x = 3$, so the cubic has the brackets $(x - 1)$ and $(x - 3)$:'),
              maths('\\begin{array}{c|ccccc} x & 0 & 1 & 2 & 3 & 4 \\\\ \\hline y & 12 & 0 & -8 & 0 & 36 \\end{array}'),
              prose('$y = 2(x + 2)(x - 1)(x - 3)$ and $y = -(x + 4)(x - 1)(x - 3)$ both have them, so test another point. At $x = 0$:'),
              working('2(2)(-1)(-3) &= 12', '-(4)(-1)(-3) &= -12'),
              prose('Only the first gives $12$. At $x = 2$ it gives $2(4)(1)(-1) = -8$, as the table does, so it is the one.'),
            ),

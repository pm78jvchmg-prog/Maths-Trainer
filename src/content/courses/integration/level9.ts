  ...
  lessons: [
    {
      ...
      slides: [
        teach(
          ...
          prose('Integrate by parts with $u = x^{n}$ and $\\frac{dv}{dx} = e^{x}$, so $\\frac{du}{dx} = nx^{n-1}$ and $v = e^{x}$:'),
        ...
        teach(
          ...
          maths('= e^{2x}\\left(\\frac{1}{2}x^{2} - \\frac{1}{2}x + \\frac{1}{4}\\right) + C'),
          prose(
            'A number in front multiplies the whole answer. A negative $k$ uses the same formula: for $e^{-x}$, $k = -1$ and $I_{n} = -x^{n}e^{-x} + nI_{n-1}$.',
          ),
        ),
    ...
    },
    {
      ...
      slides: [
        teach(
          prose(
            'Let $I_{n} = \\int_{0}^{\\frac{\\pi}{2}} \\sin^{n} x \\, dx$. Split off one $\\sin x$ and integrate by parts with $u = \\sin^{n-1} x$ and $\\frac{dv}{dx} = \\sin x$. The bracket is $0$ at both limits, which leaves',
          ),
        ...
        teach(
          prose(
            'For cosine the working swaps the roles of $\\sin$ and $\\cos$, but the numbers come out the same: $\\int_{0}^{\\frac{\\pi}{2}} \\cos^{4} x \\, dx = \\frac{3\\pi}{16}$ too.',
          ),
        ),
    ...
    },
    {
      ...
      slides: [
        ...
        teach(
          ...
          maths('= x^{2}\\left(\\frac{1}{2}(\\ln x)^{2} - \\frac{1}{2}\\ln x + \\frac{1}{4}\\right) + C'),
    ...
    },
    {
      ...
      slides: [
        teach(
          ...
          prose('For $\\sin^{n} x$, take $u = \\sin^{n-1} x$ and keep one $\\sin x$ to integrate.'),
        ),
        ...
        teach(
          ...
          maths('I_{n} = \\frac{1}{k}x^{n}e^{kx} - \\frac{n}{k}I_{n-1}'),
          maths('I_{n} = \\frac{n}{k}I_{n-1} \\quad \\left(e^{-kx}, \\text{ from } 0 \\text{ to } \\infty\\right)'),
          maths('I_{n} = \\frac{n - 1}{n}I_{n-2} \\quad \\left(\\sin^{n} x, \\cos^{n} x\\right)'),
          maths('I_{n} = \\frac{x^{m+1}(\\ln x)^{n}}{m + 1} - \\frac{n}{m + 1}I_{n-1}'),
        ),
        ask('int-red-wallis-tree'),
        ask('int-red-log-full'),

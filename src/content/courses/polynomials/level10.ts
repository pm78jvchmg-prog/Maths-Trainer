  ...
  lessons: [
    {
      ...
      slides: [
        teach(
          prose(
            'A polynomial with real coefficients that has a non-real root $z = a + bi$ also has its conjugate $\\overline{z} = a - bi$ as a root. Conjugating $p(z) = 0$ changes none of the real coefficients, so $p(\\overline{z}) = 0$ too.',
          ),
          ...
          prose('In general the sum is $2a$ and the product is $a^{2} + b^{2}$, since $i^{2} = -1$.'),
        ),
        ...
        teach(
          prose('Watch the signs when the parts are negative. For the root $-1 - 2i$ the partner is $-1 + 2i$, and'),
          working('z + \\overline{z} &= 2 \\times (-1) = -2', 'z\\overline{z} &= (-1)^{2} + (-2)^{2} = 5'),
          prose('so the factor is $x^{2} + 2x + 5$. The product is always positive. The quadratic formula agrees: $\\frac{-2 \\pm \\sqrt{-16}}{2} = -1 \\pm 2i$.'),
        ),
    ...
    },
    {
      ...
      slides: [
        teach(
          prose('A real cubic with one non-real root has its conjugate as a second, and the third root must be real. The sum of the roots finds it. $x^{3} - 5x^{2} + 11x - 15 = 0$ has the root $1 + 2i$, so $1 - 2i$ as well, and that pair adds to $2$:'),
          working('\\Sigma\\alpha &= -\\tfrac{-5}{1} = 5', '2 + \\gamma &= 5', '\\gamma &= 3'),
        ),
        ask('poly-conj-cubic-real-root'),
        ask('poly-conj-quad-factor-tiles', 2),
        teach(
          prose('The product checks it. The pair multiply to $1^{2} + 2^{2} = 5$, and the product of all three roots is $-\\frac{d}{a}$:'),
          working('5\\gamma &= -\\tfrac{-15}{1} = 15', '\\gamma &= 3'),
          prose('So the cubic factorises as $(x^{2} - 2x + 5)(x - 3)$. With a leading coefficient, divide by it as usual: $2x^{3} - 10x^{2} + 22x - 30$ has the same roots, and $\\Sigma\\alpha = -\\tfrac{-10}{2} = 5$.'),
        ),
    ...
    },
    {
      ...
      slides: [
        teach(
          prose('A real quartic with a root $1 + 2i$ has the factor $x^{2} - 2x + 5$ from the pair. Call the other factor $x^{2} + ux + v$. For $x^{4} - 3x^{3} + 5x^{2} - x - 10 = 0$,'),
          working('&(x^{2} - 2x + 5)(x^{2} + ux + v)'),
        ...
        ask('poly-conj-quad-factor-tiles', 2),
        teach(
          prose('Then solve the other factor. $x^{2} - x - 2 = (x - 2)(x + 1)$, so the four roots are $1 \\pm 2i$, $2$ and $-1$.'),
          prose('It may not factorise. If the other factor were $x^{2} + 2x + 10$, its discriminant is $4 - 40 = -36$, so its roots are another pair:'),
        ...
        teach(
          ...
          working(
            '&(x^{2} - s_1x + m_1)(x^{2} - s_2x + m_2)',
            '=\\;&x^{4} - (s_1 + s_2)x^{3}',
            '&+ (m_1 + m_2 + s_1s_2)x^{2}',
            '&- (s_1m_2 + s_2m_1)x + m_1m_2',
          ),
          prose('For $1 \\pm i$ ($s_1 = 2$, $m_1 = 2$) and $2 \\pm i$ ($s_2 = 4$, $m_2 = 5$):'),
    ...
    },
    {
      ...
      slides: [
        teach(
          prose('$x^{3} + px + q = 0$ has a root $1 + 2i$, with $p$ and $q$ real. The pair has sum $2$ and product $5$. Start from the identity with no unknown in it: there is no $x^{2}$ term, so the roots add to $0$:'),
          working('2 + \\gamma &= 0', '\\gamma &= -2'),
          prose('The pairs are $z\\overline{z}$, then $\\gamma$ with each of $z$ and $\\overline{z}$, which is $5 + \\gamma \\times 2$. So'),
          working('p &= 5 + (-2) \\times 2 = 1', 'q &= -\\alpha\\beta\\gamma = -5 \\times (-2) = 10'),
        ),
        ask('poly-conj-identity-flow'),
        ask('poly-conj-unknown-coeff'),
        teach(
          prose('If the constant is the known coefficient, start from the product instead. $x^{3} + px^{2} + qx + 15 = 0$ has a root $2 + i$, a pair with sum $4$ and product $5$:'),
          working('5\\gamma &= -15, \\quad \\gamma = -3', 'p &= -(4 - 3) = -1', 'q &= 5 + (-3) \\times 4 = -7'),
        ),
        ...
        teach(
          ...
          prose('For $x^{3} - 2x^{2} + px + q = 0$ with a root $-1 + 3i$: $s = -2$, $m = 10$, and $-2 + \\gamma = 2$ gives $\\gamma = 4$. Then $p = 10 + (-2) \\times 4 = 2$ and $q = -10 \\times 4 = -40$.'),
        ),

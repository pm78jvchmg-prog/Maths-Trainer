  ...
  lessons: [
    {
      ...
      slides: [
        ...
        teach(
          prose(
            'One curve has many parametrisations. $x = t$, $x = t + 2$ and $x = 2t$ all trace',
          ),
          display('y = x^{2} - 4x'),
          prose('With $x = 2t$, replace $x$ with $(2t)$:'),
          ...
          prose('If $x$ is $2t + 1$ instead, the point where $x = 7$ has'),
          display('2t + 1 = 7 \\qquad t = 3'),
        ),
        ask('ppar-graph-t+choice', 2),
        teach(
          prose('A number in front of $x^{2}$ multiplies the whole square. For'),
          display('y = 2x^{2} + x - 3'),
          prose('with $x = t - 1$:'),
    ...
    },
    {
      ...
      slides: [
        teach(
          ...
          prose('For $A(1, 2)$ and $B(4, -3)$ the steps are'),
          display('4 - 1 = 3 \\qquad -3 - 2 = -5'),
          prose('so the line is traced by'),
        ...
        teach(
          prose('Other values of $t$ give other points of the line. On the line from $A(1, 2)$ to $B(4, -3)$, at $t = 2$:'),
          ...
          prose('From $A(2, 1)$ to $B(6, -3)$ the line is'),
          display('x = 2 + 4t \\qquad y = 1 - 4t'),
          prose('so $t = \\frac{1}{2}$ gives the midpoint:'),
          display('x = 2 + 4(\\tfrac{1}{2}) = 4 \\qquad y = 1 - 4(\\tfrac{1}{2}) = -1'),
          prose(
            'Going backwards, solve the $x$ equation for $t$. On the line from $A(1, 2)$ to $B(4, -3)$, the point $P(-5, 12)$ has',
          ),
          display('1 + 3t = -5 \\qquad t = -2'),
          prose('and the $y$ equation checks it:'),
          display('y = 2 - 5(-2) = 12'),
          prose('$t$ is negative, so $P$ is before $A$.'),
        ),
        ask('ppar-line-point'),
    ...
    },
    {
      ...
      slides: [
        teach(
          ...
          prose('To check, substitute:'),
          display('x - a = r\\cos t \\qquad y - b = r\\sin t'),
          prose('so'),
          display('(x - a)^{2} + (y - b)^{2} = r^{2}\\cos^{2} t + r^{2}\\sin^{2} t'),
          prose('which is $r^{2}$, since'),
          display('\\cos^{2} t + \\sin^{2} t = 1'),
          prose('For'),
          display('(x - 2)^{2} + (y + 1)^{2} = 9'),
          prose('the centre is $(2, -1)$, since the signs in the brackets turn over, and the radius is $\\sqrt{9} = 3$. So'),
        ...
        askAfter(
          [
            prose('Going the other way, as in Parametric Curves: from'),
            display('x = 2 + 3\\cos t \\qquad y = -1 + 3\\sin t'),
            prose('move the constants over, square and add to get back'),
            display('(x - 2)^{2} + (y + 1)^{2} = 9'),
          ],
          'param-circle-tiles',
        ),
        teach(
          ...
          display('x = 2 + 3\\cos t \\qquad y = -1 + 3\\sin t'),
          prose(
            '$t = 0$ gives $(2 + 3, -1)$, which is $(5, -1)$, the rightmost point. $t = \\frac{\\pi}{2}$ gives $(2, -1 + 3)$, which is $(2, 2)$, the top.',
          ),
    ...
    },
    {
      ...
      slides: [
        teach(
          ...
          prose('For'),
          display('\\frac{x^{2}}{25} + \\frac{y^{2}}{9} = 1'),
          prose('$p = 5$ and $q = 3$, so'),
        ...
        teach(
          ...
          prose('A moved centre $(a, b)$ is added on, as for a circle. For'),
          display('\\frac{(x - 1)^{2}}{16} + \\frac{(y + 2)^{2}}{9} = 1'),
          prose('the centre is $(1, -2)$, $p = 4$ and $q = 3$:'),
        ...
        askAfter(
          [
            prose('And back again, as in Parametric Curves: $x = 3\\cos t$, $y = 2\\sin t$ gives'),
            display('\\frac{x^{2}}{9} + \\frac{y^{2}}{4} = 1'),
            prose('and multiplying through by $36$ gives'),
            display('4x^{2} + 9y^{2} = 36'),
          ],
          'param-ellipse-tiles',
        ),
    ...
    },
    {
      ...
      slides: [
        teach(
          ...
          prose('since'),
          display('(2at)^{2} = 4a^{2}t^{2} = 4a(at^{2})'),
          prose(
            'For $y^{2} = 12x$, $4a = 12$ gives $a = 3$. The $a$ may be negative: $y^{2} = -8x$ has $a = -2$, so $x = -2t^{2}$, $y = -4t$.',
          ),
        ...
        askAfter(
          [
            ...
            display('2k = 36 \\qquad k = 18'),
          ],
          'ppar-parabola-k',
        ),
        teach(
          prose('To pick the right parametrisation from several, substitute each one. On the circle'),
          display('x^{2} + y^{2} = 16'),
          ...
          prose('Some pairs look different and still work. $x = 4\\sin t$, $y = 4\\cos t$ gives'),
          display('16\\sin^{2} t + 16\\cos^{2} t = 16'),
          prose('too: the same circle, started from the top and run the other way. Only substituting tells you.'),
        ),
        ask('ppar-which'),

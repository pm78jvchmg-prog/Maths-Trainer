...
replacement); Conditional Probability (P(A | B) = P(A ∩ B) / P(B),
conditional branches on a tree, reading it off a Venn diagram, testing
independence with P(A | B) = P(A)).

Needs: Tree and Venn Diagrams (building them from words on the C16-widget tree
and Venn kinds: three-stage trees, three-set Venn diagrams, filling regions
from given totals); Arrangements (ordered and unordered

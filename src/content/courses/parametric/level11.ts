  ...
  lessons: [
    {
      ...
      slides: [
        ...
        askAfter(
          [
            prose('Take each term in turn. For $y^{3} + xy = 4$, replacing $y$ with $-y$ turns both terms over:'),
            display('-y^{3} - xy = 4'),
        ...
        teach(
          prose('A mirrored point can be checked by putting it in. $x^{3} + y^{2} = 12$ passes through $(2, 2)$:'),
          display('8 + 4 = 12'),
          prose('Only $y^{2}$ appears, so it is symmetric in the $x$-axis and $(2, -2)$ is on it too: $8 + 4 = 12$ again.'),
          prose('But $(-2, 2)$ is not: $-8 + 4 = -4$. The $x^{3}$ rules out the $y$-axis.'),
        ),
    ...
    },
    {
      ...
      slides: [
        ...
        teach(
          ...
          prose('Negative, so it never meets the $y$-axis. With $y = 0$ instead, $x^{2} - 6x + 5 = 0$ gives $x = 1$ and $x = 5$.'),
        ...
        ask('isk-meet-flow', 2),
        teach(
          prose('Move everything to one side before working out the discriminant. On $x^{2} + xy + y^{2} + 4y = -4$, putting $x = 0$ gives'),
          display('y^{2} + 4y + 4 = 0'),
    ...
    },
    {
      ...
      slides: [
        ...
        askAfter(
          [
            prose('On $y^{2} = 12 - 3x$ the left side is never negative, so neither is the right:'),
            display('12 - 3x \\ge 0'),
            display('x \\le 4'),
            prose('The largest $x$ is $4$, where $y = 0$. On $y^{2} = 3x - 12$ the same step gives $x \\ge 4$, the smallest $x$.'),
          ],
          'isk-furthest',
        ),
        ...
        teach(
          prose(
            'When the right-hand side is a product, find where it is negative. On $y^{2} = x(x - 3)$ it is zero at $x = 0$ and $x = 3$. Between them $x > 0$ and $x - 3 < 0$, so the product is negative and there are no points.',
          ),
        ...
        teach(
          prose(
            'A factor written the other way round turns this over. On $y^{2} = (x + 1)(3 - x)$ both brackets are positive between $-1$ and $3$, and one is negative outside, so the curve lives in',
          ),
          display('-1 \\le x \\le 3'),
    ...
    },
    {
      ...
      slides: [
        teach(
          ...
          prose('On $x^{2} + xy + y^{2} = 3$, $p = q = r = 1$, so'),
          display(DYDX + ' = -\\frac{2x + y}{x + 2y}'),
          prose('Horizontal where $2x + y = 0$, which is the line $y = -2x$.'),
        ),
        ask('isk-flat-line'),
        askAfter(
          [
            prose('To find the points, put the line into the curve. With $y = -2x$ in $x^{2} + xy + y^{2} = 3$:'),
        ...
        teach(
          prose(
            'Vertical tangents come from the bottom. On $x^{2} + xy + y^{2} = 3$ the bottom $x + 2y$ is zero on the line $x = -2y$. Write it as $x = \\ldots$ and put it in for $x$:',
          ),
        ...
        teach(
          prose(
            'Check the other part of the fraction is not zero at the point as well, or the gradient is not defined there at all. At $(1, -2)$ the bottom is $1 - 4 = -3$, so the tangent there really is horizontal.',
          ),
        ),
    ...
    },
    {
      ...
      slides: [
        teach(
          ...
          prose('A is $x^{2} + y^{2} = 9$, a **circle**: symmetric in both axes, meeting each axis at $\\pm 3$.'),
          prose(
            'B is $\\frac{x^{2}}{16} + \\frac{y^{2}}{4} = 1$, an **ellipse**: $y = 0$ gives $x = \\pm 4$ and $x = 0$ gives $y = \\pm 2$, so it is wider than it is tall.',
          ),
          prose(
            'C is $\\frac{x^{2}}{4} - \\frac{y^{2}}{9} = 1$, a **hyperbola**: it meets the $x$-axis at $\\pm 2$ and never the $y$-axis, and $x^{2} \\ge 4$, so it comes in two branches.',
          ),
          prose('D is $y^{2} = 4x$, a **parabola**: symmetric in the $x$-axis only, through the origin, and $x \\ge 0$, so it opens to the right.'),
        ),
        ask('isk-match'),
        ask('isk-sketch-flow'),
        ...
        teach(
          prose(
            'Two curves of the same kind are told apart by their crossings. $\\frac{x^{2}}{9} + \\frac{y^{2}}{25} = 1$ meets the axes at $x = \\pm 3$ and $y = \\pm 5$: taller than it is wide. Swapping the $9$ and the $25$ gives the wider one.',
          ),
          prose('Written without fractions it is $25x^{2} + 9y^{2} = 225$, and the bounds come out the same way. $9y^{2} \\ge 0$, so'),
        ...
        teach(
          prose(
            'For a hyperbola only the number under $x^{2}$ shows on the axis. $\\frac{x^{2}}{9} - \\frac{y^{2}}{4} = 1$ meets it at $\\pm 3$, and $\\frac{x^{2}}{4} - \\frac{y^{2}}{9} = 1$ at $\\pm 2$.',
          ),
        ),
